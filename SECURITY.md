...
- **Two concurrent runs can still collide on one network slot.** If a stale lockfile is present (a run that was `kill -9`'d) and two runs claim at the same instant, both can take the same tap and `/30`; the loser's VM fails opaquely when firecracker cannot open the tap. It is a reliability bug rather than a crossing of the isolation boundary — the two runs get different VMs, and the loser dies rather than sharing anything — but it is real, and the fix is a `flock`-based claim rather than the current staleness heuristic.
