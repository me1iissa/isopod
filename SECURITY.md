...
- **A network slot has one run on it.** A slot is claimed by an exclusive `flock` held for the run's lifetime, so no timing, no crash, and no concurrency gets two runs onto one tap and `/30`. The kernel releases the lock when the owning process dies — `kill -9` included — so the slot is reclaimable immediately with nothing to sweep and no liveness guess.
